...
pub fn scan_source(source: &str) -> Vec<TokenData> {
    let tokenizer = scan::Tokenizer::new(source);
    tokenizer.map(process_token_kw).collect()
}

fn process_token_kw(tok: TokenData) -> TokenData {
    match tok.kind {
        Token::Ident => {
...
lazy_static! {
    ...
    static ref _TOKEN_KW_TABLE: HashMap<&'static str, Token> =
        _TOKEN_KW_LIST.iter().cloned().collect();
}
