...
use crate::ast::token::{LitKind, Token, TokenData, TokenLoc};
...
impl<'s> Iterator for Tokenizer<'s> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let tok_start = self.current_loc;
        loop {
            ...
            match next_action.emit_token {
                ...
                None => {
                    if self.current_state == LexState::Eof {
                        return None;
                    }
                }
            }
...
fn lex_next_chunk(state: LexState, chunk: Option<char>, rest: &str) -> LexNext {
    use LexState::*;
