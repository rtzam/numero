...
use inkwell::targets::{TargetMachine, Target, FileType, CodeModel, RelocMode, InitializationConfig,};
...
pub fn build_file(filename: &str, opt_level: OptimizationLevel, emit: Option<NoshEmit>){
    ...
    match emit{
        ...
        Some(NoshEmit::Asm) => {
            ...
            let buffered_asm = tm.write_to_memory_buffer(&llmod, FileType::Assembly)
...
fn prep_llvm_target_machine(opt_level: OptimizationLevel) -> TargetMachine{
    ...
    let target_machine = target.create_target_machine(
        &triple,
        cpu.to_str().expect("TargetMachine::get_host_cpu_name() failed"),
        cpu_info.to_str().expect("TargetMachine::get_host_cpu_features() failed"),
        opt_level,
        reloc,
        model,
